...
GAGENT_PROFILE=${GAGENT_PROFILE:=}   # The config profile to apply
GCLOUD_PASSWORD=${GCLOUD_PASSWORD:=} # Grafana cloud password
GCLOUD_METRICS_USER=${GCLOUD_METRICS_USER:=} # user to upload metrics
GCLOUD_LOGS_USER=${GCLOUD_LOGS_USER:=}
GCLOUD_TRACES_USER=${GCLOUD_TRACES_USER:=}
...
main() {
  ...
  # Write alll of the required Env Vars to the systemd service env file
  echo "GCLOUD_PASSWORD=${GCLOUD_PASSWORD}" >> /etc/default/grafana-agent
  ...
  sed -i 's/CUSTOM_ARGS="/CUSTOM_ARGS="-config.expand-env /' /etc/default/grafana-agent
